...
#         self.dropout = torch.nn.Dropout(0.1)
    ...
    def forward(self, inputs, inference=False):
        ...
        #     #x1 = text_layer(x, y, extend_text_masks, extend_image_masks)
        #     text_embeds = text_layer(text_embeds, image_embeds,extend_text_masks,extend_image_masks)[0]
        ...
        #text_feats = (text_embeds * title_mask.unsqueeze(-1)).sum(1) / title_mask.sum(1).unsqueeze(-1)
        ...
        prediction = self.classifier(self.dropout(image_feats))
        # prediction = self.classifier(self.dropout(text_feats))
        if inference:  # 预测
