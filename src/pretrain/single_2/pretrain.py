...
from qqmodel.qq_uni_model import Single_Stream_Model_Pretrain
from optim.create_optimizer import create_optimizer,build_optimizer
from utils.eval_spearman import evaluate_emb_spearman
from utils.utils import set_random_seed,setup_logging
...
def get_pred_and_loss(model, item, task=None):
    ...
    masked_lm_loss, loss_ita, loss_itm,masked_mfm_loss = model(video_feature, video_mask, input_ids, attention_mask, text_tokne_type,video_token_type)
...
def train(model, train_loader,optimizer,pretrain_sampler, get_pred_and_loss,batch_size,scheduler=None, num_epochs=5,device=None):
    ...
    for epoch in range(num_epochs):
        ...
        for item in pbar:
            ...
            optimizer.zero_grad()
            with autocast():
                masked_lm_loss,itc_loss,itm_loss,masked_mfm_loss= get_pred_and_loss(model, item)
                loss = masked_lm_loss + itc_loss/2.0 + itm_loss + masked_mfm_loss
            ...
            pbar.set_postfix(device = device,epoch=epoch,total_loss = train_loss.avg ,itm_loss = itm_losses.avg,mlm_loss = mlm_losses.avg ,itc_loss = itc_losses.avg,mfm_loss=mfm_losses.avg,lr=optimizer.param_groups[4]['lr'])
            if device==0:
                if (step + 1) % 500 == 0 and step > 0:
                    ...
                    logging.info(f"Epoch={epoch + 1}/{num_epochs} \t step={step:3} \t total_loss={train_loss.avg:6.4},itm_loss={itm_losses.avg:6.4},mlm_loss={mlm_losses.avg:6.4},itc_loss={itc_losses.avg:6.4},mfm_loss={mfm_losses.avg},lr[0]={optimizer.param_groups[0]['lr']}")
        #ema.apply_shadow()
        if device==0:
            torch.save(model.state_dict(), f'./checkpoint/model_epoch_{epoch}.pth')
...
if __name__ == "__main__":
    ...
    if args.local_rank==0:
        logging.info(f"len(train_loader)={len(pretrain_dataloader)}")
        delta_mem = psutil.Process(os.getpid()).memory_info()[0] / 2. ** 30 - m0
        logging.info(f"Dataset used memory = {delta_mem:.1f}GB")
    total_steps = NUM_EPOCHS * (len(pretrain_dataloader.dataset)) // BATCH_SIZE
    ...
    model = Single_Stream_Model_Pretrain(args.bert_dir).cuda()
    
    optimizer,scheduler = build_optimizer(args, model,len(pretrain_dataloader),logging)
