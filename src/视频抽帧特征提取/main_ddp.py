...
def train_and_validate(args, train_dataloader, val_dataloader, train_sampler,validation_sampler,fold,device):
    ...
    # model = VLModel(args)
    model = Clip_Vit_Cross_Model(args).cuda()
    ...
    for epoch in range(args.max_epochs):
        ...
        if device==0:
            ...
            logging.info(f"not use ema validate Epoch {epoch} step {step}: loss {loss:.3f}, {results}")
        ...
        if device==0:
            if mean_f1_ema3 > 0.66:
                ...
                torch.save({'epoch': epoch, 'model_state_dict': state_dict, 'mean_f1': mean_f1_ema3},
                           f'{args.savedmodel_path}/model_macbert_seed_2012_ema0.999_epoch_{epoch}_{step}_mean_f1_{mean_f1_ema3}.bin')
        ema3.restore()
    ...
    # logging.info(f"swa Epoch {epoch} step {step}: loss {loss:.3f}, {results}")
    # print(f"swa Epoch {epoch} step {step}: loss {loss:.3f}, {results}")
    # state_dict = {k: v for k, v in swa_model.module.state_dict().items() if 'relative_positions' not in k}
    # torch.save({'epoch': epoch, 'model_state_dict': state_dict, 'mean_f1': mean_f1},
    #            f'{args.savedmodel_path}/fold_{fold}/model_swa_last_three_model_mean_f1_{mean_f1}_.bin')
