...
# def setup_logging(args):
#     log_name = f'learning_rate_{args.learning_rate}_vison_learning_rate_{args.vison_learning_rate}_classifier_learning_rate_{args.classifier_learning_rate}_bert_seq_length_{args.bert_seq_length}_max_frames_{args.max_frames}_fgm_{args.fgm}_num_top_layer_{args.num_top_layer}_fold_{args.fold}.log'
#     logging.basicConfig(format='%(asctime)s - %(levelname)s - %(message)s',
#                         filename=log_name,
#                         filemode='w',  ##模式，有w和a，w就是写模式，每次都会重新写日志，覆盖之前的日志
#                         datefmt='%Y-%m-%d %H:%M:%S',
    ...
    *************************************************************************************************************
    ...
    for i in range(args.num_top_layer-1,-1,-1):
        if i != 1:    #主要在于layer.1和layer.10 layer.11这两个，匹配的话，不加if这个判断会导致在i = 1的时候，将layer.10的参数也包含进来了
            ...
            tmp = [
            {'params': [p for n, p in model.named_parameters() if not any(nd in n for nd in no_decay) and 'cross_modal' in n and 'layer.1' in n],'weight_decay': args.weight_decay,'lr':args.learning_rate*epsion},

            {'params': [p for n, p in model.named_parameters() if any(nd in n for nd in no_decay) and 'cross_modal' in n and 'layer.1' in n], 'weight_decay': 0.0,'lr':args.learning_rate*epsion}]
            optimizer_grouped_parameters.extend(tmp)

            tmp =  [n for n, p in model.named_parameters() if not any(nd in n for nd in no_decay) and 'cross_modal' in n and 'layer.1' in n]

            optimizer_grouped_parameters_name.extend(tmp)    
            tmp2 = [n for n, p in model.named_parameters() if any(nd in n for nd in no_decay) and 'cross_modal' in n and 'layer.1' in n]
        epsion *= 0.93
    '''
    *************************************************************************************************************
    *************************************************************************************************************
