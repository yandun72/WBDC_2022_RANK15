...
def inference():
    ...
    # 2. load model
    now=datetime.datetime.now()
    model = Clip_Vit_Cross_Model_finetune(args,is_inference=True)
    checkpoint = torch.load('../../finetune/double_2/save/model_macbert_seed_2012_ema0.999_epoch_3_fold_0.bin', map_location='cpu')
    ...
    np.save(f'../double_2_device_{device}.npy',now_device_pred)
