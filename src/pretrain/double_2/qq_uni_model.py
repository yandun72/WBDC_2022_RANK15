...
class QQUniModel(nn.Module):
    def __init__(self, args, task=['itm', 'mfm','itc']):
        super().__init__()
        # 加载bert_config模型
    ...
    def forward(self, video_feature, video_mask, text_input_ids, text_mask,task=['itm', 'mfm','mlm']):
        ...
        video_features,video_text_cls,text_cls,image_cls = self.roberta(video_feature, video_mask, text_input_ids, text_mask)
        ...
        if 'itc' in sample_task:
            ...
            logits_per_text = torch.matmul(text_cls, image_cls.t()) * logit_scale
        ...
        if 'itm' in sample_task:
            pred = self.newfc_itm(video_text_cls)
            itm_loss = nn.BCEWithLogitsLoss()(pred.view(-1), video_text_match_label.view(-1))
            loss += itm_loss
            
        return (pred, loss, itm_loss, masked_vm_loss,itc_loss)
