...
def train_and_validate(args, train_dataloader, val_dataloader, train_sampler,validation_sampler,fold,device):
    ...
    # model = VLModel(args)
    model = Clip_Vit_Cross_Model_finetune(args).cuda()
    ...
    for epoch in range(args.max_epochs):
        ...
        for batch in tqdm(pbar):
            ...
            if (step + 1) % args.accumlate_step ==0:
                ...
                scaler.update()
                ema3.update()
            if (step + 1) % 1500 == 0 and step >= 6000 and device == 0:
    ...
    if device == 0:
        ...
        print('*********************use swa validate*************************')
        logging.info('*********************use swa validate*************************')
        results = validate(swa_model, val_dataloader,validation_sampler,device)
        results = {k: round(v, 4) for k, v in results.items()}
        ...
        torch.save({'epoch': epoch, 'model_state_dict': state_dict, 'mean_f1': mean_f1},
                   f'{args.savedmodel_path}/model_swa_last_three_model_mean_f1_{mean_f1}_.bin')
