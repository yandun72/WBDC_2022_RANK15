...
def train_and_validate(args, train_dataloader, val_dataloader, train_sampler,validation_sampler,fold,device):
    ...
    scaler = torch.cuda.amp.GradScaler()
    autocast = torch.cuda.amp.autocast
    # swa_model,swa_model_2,swa_model_3 =None,None,None
    step = 0
    cnt = 0
    #swa_model_3 = torch.optim.swa_utils.AveragedModel(model)
...
#             #if step > 14*3 and (step + 1) % 5 == 0 and device ==0:
...
#                            f'{args.savedmodel_path}/swa_tmp/no_ema/model_noema_{epoch}_step_{step}_cnt_{cnt}.bin')
#                 ema3.apply_shadow()
                
#                 state_dict = {k: v for k, v in model.module.state_dict().items() if 'relative_positions' not in k}
#                 torch.save({'model_state_dict': state_dict},
#                            f'{args.savedmodel_path}/swa_tmp/has_ema/model_hasema_{epoch}_step_{step}_cnt_{cnt}.bin')
#                 ema3.restore()
#                 cnt += 1
            step += 1
...
#         if epoch >= 2 and device == 0:
#             state_dict = {k: v for k, v in model.module.state_dict().items() if 'relative_positions' not in k}
#             torch.save({'model_state_dict': state_dict},
#                        f'{args.savedmodel_path}/swa_tmp_last_three_epoch/no_ema/model_noema_{epoch}.bin')
#             ema3.apply_shadow()
                
#             state_dict = {k: v for k, v in model.module.state_dict().items() if 'relative_positions' not in k}
#             torch.save({'model_state_dict': state_dict},
#                        f'{args.savedmodel_path}/swa_tmp_last_three_epoch/has_ema/model_hasema_{epoch}.bin')
#             ema3.restore()
...
def parse_args_for_ddp():
    ...
    parser.add_argument('--log_name',default=-1,type=str,help='node rank for distributed training')
    parser.add_argument('--epoch',default=-1,type=int,help='node rank for distributed training')
...
if __name__ == "__main__":
    ...
    args.max_epochs = args_ddp.epoch
    log_name = args_ddp.log_name
