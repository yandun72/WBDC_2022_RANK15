...
class VLModel(nn.Module):
    def __init__(self, args,is_test=False):
        super().__init__()
        if is_test != True:
            pretrain_state = torch.load('/home/tione/notebook/env/code/pretrain/single_stream/1_xxx(plan name)/checkpoint/model_epoch_5_step_137496_total_loss_ 4.867.pth', map_location='cpu')
    ...
    def forward(self, inputs, inference=False):
        ...
        if inference:  # 预测
            ...
            return self.cal_loss(prediction,prediction_m, inputs['label'])

    @staticmethod
    def cal_loss(prediction, prediction_m,label):
