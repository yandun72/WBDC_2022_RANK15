...
if __name__ == "__main__":
    ...
    if args.local_rank==0:
        ...
        setup_logging_2(args,f'验证3个epoch的模型效果_{seed}.log')
    ...
    for i, (train_index, vali_index) in enumerate(kf.split(anns, anns.category_id)):
        ...
        state = torch.load('./save/model_macbert_seed_2012_ema0.999_epoch_2_fold_0_mean_f1_0.7215.bin')['model_state_dict']
        ...
        '''
        model = Single_Model_Finetune(args).cuda()
        state = torch.load('./save/model_macbert_seed_2012_ema0.999_epoch_3_fold_0_mean_f1_0.7258.bin')['model_state_dict']
        model.load_state_dict(state)
        model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)  
        model = torch.nn.parallel.DistributedDataParallel(model,
                                                          device_ids=[device],find_unused_parameters=True)
        results = validate(model, val_dataloader,validation_sampler,device)
        if device==0:
            logging.info(f"*********************epoch 3*********************")
            print('*********************epoch 3*********************')
            print('result:', results)
            logging.info(f"epoch 3, {results}")
        ...
        '''
        model = Single_Model_Finetune(args).cuda()
        state = torch.load('./save/model_macbert_seed_2012_ema0.999_epoch_4_fold_0_mean_f1_0.7219.bin')['model_state_dict']
        model.load_state_dict(state)
        model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)  
        model = torch.nn.parallel.DistributedDataParallel(model,
                                                          device_ids=[device],find_unused_parameters=True)
        results = validate(model, val_dataloader,validation_sampler,device)
        if device==0:
            logging.info(f"*********************epoch 4*********************")
            print('*********************epoch 4*********************')
            print('result:', results)
            logging.info(f"epoch 4, {results}")
